...
[[nodiscard]] static std::string genTxnId() {
    ...
    oss << "pos-txn-" << ms << "-" << std::uniform_int_distribution<>(1000,9999)(g);
    return oss.str();
}
...
[[nodiscard]] static json purchase(const json& data, POSContext& ctx) {
    try {
        if (!data.contains("card")) return err("ERR_MISSING_CARD");
        ...
        if (amount <= 0)           return err("ERR_INVALID_AMOUNT","Amount must be > 0");
        if (amount > POS_MAX_SINGLE) return err("ERR_SINGLE_LIMIT","Exceeds POS single limit");

        // Decrypt PAN + verify PIN (logic unchanged per client requirement)
        std::string pan, expiry = card.value("expiry",""), cvv = card.value("cvv","");
        if (card.contains("pan") && card.contains("pin")) {
            pan = PANEncryptionService::getInstance().decryptPAN(card["pan"]);
            if (!PINService::getInstance().verifyPIN(pan, card["pin"].get<std::string>()))
                return err("ERR_INVALID_PIN","PIN verification failed");
        } else {
            pan = card.value("pan","");
        }

        auto cardRes = ctx.cards.select("account_number","scheme","status")
            .where("pan=:p AND expiry=:e").bind("p",pan).bind("e",expiry).execute();
        if (cardRes.count() == 0) return err("ERR_CARD_NOT_FOUND");

        Row r = cardRes.fetchOne();
        std::string accNo  = r[0].get<std::string>();
        std::string scheme = r[1].isNull() ? "" : r[1].get<std::string>();
        if (r[2].get<std::string>() != "ACTIVE") return err("ERR_CARD_INACTIVE");
        ...
        if (balance < amount + fee) return err("ERR_INSUFFICIENT_FUNDS");

        ctx.sess->startTransaction();
        try {
            ctx.accounts.update().set("balance", balance-(amount+fee))
            ...
            auto ins = ctx.posTbl.insert(
                ...
                .values(txnId,clientId,merchantId,terminalId,location,
                        accNo,amount,fee,pan,scheme,
                        "SUCCESS","POS purchase successful",nullptr,0.0,"N")
                .execute();

            ctx.master.insert("table_name","reference_id","status")
                .values("transaction_pos", ins.getAutoIncrementValue(), "SUCCESS").execute();

            ctx.sess->commit();

            try { ctx.cards.update().set("last_transaction_time",mysqlx::expr("NOW()"))
                    .where("pan=:p").bind("p",pan).execute(); } catch (...) {}
        ...
        } catch (...) { ctx.sess->rollback(); return err("ERR_DB_FAILURE"); }
...
[[nodiscard]] static json refund(const json& data, POSContext& ctx) {
    try {
        ...
        if (origTxnId.empty()) return err("ERR_INVALID_ORIG_TXN","origTransactionId required");

        auto pRes = ctx.posTbl.select("id","amount","account_number","card_pan",
                                       "refunded_amount","flag","message")
            .where("transaction_id=:tid").bind("tid",origTxnId).execute();
        if (pRes.count() == 0) return err("ERR_PURCHASE_NOT_FOUND");
        ...
        if (msg  != "POS purchase successful") return err("ERR_INVALID_REFUND_SOURCE");
        if (flag != "N")                       return err("ERR_ALREADY_REFUNDED");
        if (amount <= 0 || amount > purchAmt)  return err("ERR_INVALID_REFUND_AMOUNT");

        double balance = ctx.accounts.select("balance")
            .where("account_number=:a").bind("a",acc).execute().fetchOne()[0].get<double>();
        Row feeRow = ctx.posTbl.select("fee").where("id=:id").bind("id",id).execute().fetchOne();
        double fee = feeRow[0].isNull() ? 0.0 : feeRow[0].get<double>();

        if (amount <= fee) return err("ERR_AMOUNT_LESS_THAN_FEE");
        double netRefund = amount - fee;

        ctx.sess->startTransaction();
        try {
            ctx.accounts.update().set("balance", balance+netRefund)
                .where("account_number=:a").bind("a",acc).execute();

            std::string newFlag = (amount == purchAmt) ? "RF" : "PR";
            ctx.sess->sql("UPDATE transaction_pos SET refunded_amount=?,flag=? WHERE id=?")
                .bind(amount).bind(newFlag).bind(id).execute();
            ...
            auto ins = ctx.posTbl.insert(
                ...
                .values(txnId,clientId,merchantId,terminalId,location,
                        acc,amount,0.0,pan,"SUCCESS","POS refund successful",id)
                .execute();
